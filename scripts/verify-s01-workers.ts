...
type Mode = 'full' | 'dry-run' | 'prepare-only' | 'readiness-only' | 'help'

function modeFromArgs(args: string[]): Mode {
  const modes = args.filter(arg => arg.startsWith('--')).filter(arg => ['--dry-run', '--prepare-only', '--readiness-only', '--help'].includes(arg))
  if (modes.length > 1 || args.some(arg => arg.startsWith('--') && !modes.includes(arg))) {
    throw new Error('Only one of --dry-run, --prepare-only, --readiness-only, or --help is supported.')
  }
  return modes[0]?.slice(2) as Mode ?? 'full'
}

function help() {
  console.log(`Usage: npm run verify:s01:workers [-- --dry-run|--prepare-only|--readiness-only|--help]
...
function seedSql(): string {
  ...
  return [
    `INSERT INTO users (id, username, password_hash, created_at) VALUES ('s01-owner', 's01-owner', 'unused', ${sql(createdAt)});`,
...
async function main() {
  ...
  try {
    ...
    console.log(`Prepared isolated local D1 at ${resolvedPersist}`)

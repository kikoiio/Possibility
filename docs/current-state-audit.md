...
- AC10 共享人物旧数据矩阵扩展 `legacy-compat.test.ts`：同一居民在两个世界拥有各自的固定姓名/模型、地点状态、旧记忆和聊天记录；显式 timeline context 与聊天历史 API 只返回对应世界内容。AC06 新增睡眠/忙碌/两者兼有/异地的无副作用拒绝矩阵，以及跨 UTC 午夜睡眠日程边界测试。AC07 新增提议→接受→赴约窗口拒绝→到点履约→归来回顾完整旅程，逐项核验事件、事实、人物状态和关系记忆。当前全量 API 226/226、Web 9/9，API/Web build 均通过。
